    ...
    for itemXunHuan in list:
        ...
        try:
            ...
            if doubleHighArray[-1] > ma5[-1] > doubleOpenArray[-1] and ma5[-2] < ma5[-3] < ma5[-4] \
                    and doubleCloseArray[-1] > doubleOpenArray[-1]:
                if zhouqi == 'W':
                    common_image.plt_image_tongyichutu_2(codeItem,
                                                         "W",
                                                         "【" + keyword + "】_" + "跨越5周线",
...
asyncio.get_event_loop().run_until_complete(
    main("http://cwzx.shdjt.com/cwcx.asp?gdmc=%CF%C8%BD%F8%D6%C6%D4%EC%B2%FA%D2%B5%CD%B6%D7%CA%BB%F9%BD%F0%28%D3%D0%CF%DE%BA%CF%BB%EF%29", "先进制造", "M"))
asyncio.get_event_loop().run_until_complete(
    main("http://cwzx.shdjt.com/cwcx.asp?gdmc=%CF%C8%BD%F8%D6%C6%D4%EC%B2%FA%D2%B5%CD%B6%D7%CA%BB%F9%BD%F0%28%D3%D0%CF%DE%BA%CF%BB%EF%29", "先进制造", "W"))
