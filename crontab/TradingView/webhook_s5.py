...
def exec_d(codeItem, zhangdiefu, price, codeName):
    ...
    re = get_industry(codeItem)

    table_item_data = [codeName, zhangdiefu, price + " " + re, ma10_60[-3], ma10_60[-2], ma10_60[-1], state_60, ma10[-3], ma10[-2],
                       ma10[-1],
...
def get_industry(codeItem):
    ...
    if len(data) > 0:
        ...
        if "ST" in codeName:
            return ""
        if len(plate) > 0:
            json_list = json.loads(plate, strict=False)
