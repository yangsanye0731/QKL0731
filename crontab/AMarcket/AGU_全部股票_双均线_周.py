        ...
        try:
            ...
            if ma10[-1] > sma10[-1] and ma10[-2] < sma10[-2]:
                ...
                common_notion.create_content(database_id=database_id, title=codeName,
                                             ce_lve_lei_xing='10周双均线金叉', tu_pian=image_url,
            ...
            if ma60[-1] > sma60[-1] and ma60[-2] < sma60[-2]:
                ...
                logging.debug("图片URL：%s", image_path)
                common_notion.create_content(database_id=database_id, title=codeName,
                                             ce_lve_lei_xing='60周双均线金叉', tu_pian=image_url,
                                             mark=mark, gai_nian=gai_nian, code=codeItem, create_time=time_str)
            ...
            if ma144[-1] > sma144[-1] and ma144[-2] < sma144[-2]:
                ...
                logging.debug("图片URL：%s", image_path)
                common_notion.create_content(database_id=database_id, title=codeName,
                                             ce_lve_lei_xing='144周双均线金叉', tu_pian=image_url,
                                             mark=mark, gai_nian=gai_nian, code=codeItem, create_time=time_str)
            ...
            print(e)
            traceback.print_exc()
            # common.dingding_markdown_msg_03("触发AGU_全部股票_双均线_周执行异常", "触发AGU_全部股票_双均线_周执行异常")
...
if __name__ == "__main__":
    ...
    if num_args < 1:
        ...
        database_id = "4847a0b20fe045b88194cc378c450fb2"
        count_result_b, count_result_e = strategy('W', time_str1, database_id)
        ...
        common.dingding_markdown_msg_02("触发AGU_全部股票_双均线_周执行完成", "触发AGU_全部股票_双均线_周执行完成")
    else:
        logging.info("开始策略回测逻辑")
        # Notion数据库ID：任务跟踪（Auto）（回测）
        database_id = "3fe08fb1426a4093a88e19d4584e6610"
        ...
        while current_date < end_date:
            ...
            strategy(zhouqi='W', endstr=time_str1, database_id=database_id)
