...
async def index(page, cookie1, url, codeName):
    try:
        ...
        if openArray[-1] < MA_20[-1] and closeArray[-1] > MA_20[-1]:
            image_path = common_image.plt_image_tongyichutu_zhishu_xueqiu(data_history['close'], codeItem, codeName,
                                                                          "D",
                                                                          "【01雪球指数】均线跨过20天均线",
                                                                          "【01雪球指数】均线跨过20天均线" + time_str,
                                                                          str(zhangdiefu[-1]),
                                                                          "%.2f" % huanshoulv[-1])
            image_url = "http://47.240.11.144/" + image_path[6:]
            common.dingding_markdown_msg_02('触发【01雪球指数】均线跨过20天均线' + time_str + codeName + '(' + codeItem + ')',
                                            '触发【01雪球指数】均线跨过20天均线' + time_str + codeName + '(' + codeItem + ')'
                                            + "\n\n> ![screenshot](" + image_url + ")")


    except (IOError, TypeError, NameError, IndexError, TimeoutError, Exception) as e:
        common.dingding_markdown_msg_02('触发【01雪球指数】均线跨过20天均线' + codeName + '(' + codeItem + ')报错了 ！！！！！！',
                                        '触发【01雪球指数】均线跨过20天均线' + codeName + '(' + codeItem + ')报错了 ！！！！！！')
        print(e)
